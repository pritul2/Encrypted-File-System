...
public class EFS extends Utility {
    ...
    public void write(String file_name, int starting_position, byte[] content, String password) throws Exception {
        ...
        for (int i = start_block + 1; i <= end_block + 1; i++) {
            ...
            if (i == start_block + 1 && starting_position != start_block * Block_size) {
                ...
                byte[] encrypted_text_unpad = Arrays.copyOfRange(encrypted_prefix, 0, Block_size);
                byte[] plain_prefix = decript_AES(encrypted_text_unpad, pwd_base_key);
            ...
            if (i == end_block + 1) {
                ...
                if (end.exists()) {
                    ...
                    byte[] plain_postfix = decript_AES(encrypted_postfix_unpad, pwd_base_key);
        ...
        if (starting_position + len > length(file_name, password)){
            ...
            byte[] pwd_key = deriveKeyFromPassword(padded_password, get_salt);
    ...
    public void cut(String file_name, int length, String password) throws Exception {
        ...
        // Verify password
         ...
         int Block_size = 912;
         int end_block = (length) / Block_size;
         salt[last_index] += end_block+1;
 
         byte[] encrypted_file = read_from_file(new File(root, Integer.toString(end_block + 1)));
         byte[] encrypted_file_unpad = Arrays.copyOfRange(encrypted_file, 0, Block_size);
         byte[] pwd_base_key = deriveKeyFromPassword(padded_password, salt);
         byte[] plain_text = decript_AES(encrypted_file_unpad, pwd_base_key);
         String str = new String(plain_text, StandardCharsets.UTF_8);
 
         str = str.substring(0, length - end_block * Block_size);
         ...
         save_to_file(combinedAndPadded, new File(root, Integer.toString(end_block + 1)));
    ...
    /*public static void main(String[] args) {
         ...
         EFS efs = new EFS(edr);
         try {
             //efs.create("my_file10", "HelloWORLD", "macbook");
             //System.out.println(efs.findUser("my_file10"));
             //System.out.println(efs.length("my_file10","macbook"));
             //efs.write("my_file10",0,getContent().getBytes(),"macbook");
             efs.write("my_file10",900,"Hello world".getBytes(),"macbook");
             efs.read("my_file10",900,50,"macbook");
             //System.out.println(efs.check_integrity("my_file10","macbook"));
         } catch (Exception e) {
