...
public class EFS extends Utility{
    ...
    public void create(String file_name, String user_name, String password) throws Exception {
        ...
        if (user_name.length() > 128 || password.length() > 112) {
        ...
        String salted_password = password + new String(salt);
        String padded_password = padString(salted_password, 128);
        
        // Adding padded username and password salt to header
        byte[] header = (padded_user_name + salted_password).getBytes(StandardCharsets.US_ASCII);

        // Creating password hash and getting file length
        byte[] pwd_hash = hash_SHA256(padded_password.getBytes(StandardCharsets.US_ASCII));
        byte[] file_length = longToBytes(metadata_file.length());

        // Creating secret data array of length pwd_hash + file_length
        byte[] secret_data = new byte[pwd_hash.length + file_length.length];

        // Adding password hash and File length to secret data
        System.arraycopy(pwd_hash, 0, secret_data, 0, pwd_hash.length);
        System.arraycopy(file_length, 0, secret_data, pwd_hash.length, file_length.length);
        ...
        byte[] metadata = new byte[header.length+secret_data.length];
    ...
    public String findUser(String file_name) throws Exception {
        FileInputStream metadata_input = new FileInputStream(file_name + ".metadata");
    	return null;
    }
    ...
    public static void main(String [] args){
        ...
        EFS efs = new EFS(edr);
        try{
        efs.create("my_file","pritul","pritul123");
        }
        catch(Exception e){
